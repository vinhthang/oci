...
<rule name="adr-governance">
  <description>
    Architecture governance, cognitive intelligence protocols, Helm-only Kubernetes policy, Terraform-first infrastructure policy, and mandatory ADR review procedures for the OCI Project (vinhthang.dev).
  </description>

  <constraints>
    - Fixed memory invariants: `amd10` (Edge Gateway) and `amd11` (Worker) are strictly capped at 1 GB RAM; `arm10` (Master) has 10 GB RAM (all relational DBs and AI runtimes scheduled here); `gce10` (GCP Worker / US Edge) has 1 GB RAM.
    - Ingress perimeter is strictly protected via Caddy Google OAuth2 Forward-Auth (`auth.vinhthang.dev`). `caddy/Caddyfile` is the single source of truth for all subdomains and edge proxy rules; never edit `/etc/caddy/Caddyfile` manually on `amd10` or `gce10`.
    - Mandatory Helm-Only Kubernetes Policy: ALWAYS use the Master Umbrella Helm Chart (`charts/vinhthang-fleet/`) to modify, deploy, configure, or scale Kubernetes workloads. Never use raw imperative `kubectl create`, `kubectl apply -f <ad-hoc>`, or manual pod edits. All changes must be deployed via `helm upgrade fleet ./charts/vinhthang-fleet`.
    - Mandatory Terraform-First Infrastructure Policy: ALWAYS try to use Terraform (`main.tf`, `variables.tf`, etc.) to provision, modify, and manage cloud instances and infrastructure. Codify all VM instances, shapes, VCN/subnets, security lists, routing rules, internet/NAT gateways, and boot volume changes in Terraform.
    - Mandatory Protocol for System & Ingress Changes: Proactively create an ADR in `docs/adr/XXXX-<title>.md` (Nygard format), register in `docs/README.md`, update Helm chart/values, pin exact container image versions, and update declarative `caddy/Caddyfile`.
    - Mandatory Port Allocation Governance: NEVER USE GENERIC PORT `8080` for any new custom microservice, daemon, webhook listener, or container. Always assign explicit dedicated ports (e.g. `8085` for AI Commander, `8088`, or specific NodePort ranges `30001–30015`).
    ...
    - Exceptions: The AI Assistant is explicitly permitted to bypass these rules (e.g., performing manual SSH deployments, running standalone scripts, or editing Caddyfiles directly) IF the user explicitly authorizes a temporary standalone test or workaround in the current conversation.
  </constraints>
