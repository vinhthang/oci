<rule name="agent-delegation">
  ...
  <constraints>
    ...
    - Spawn execution subagents with `Model: "flash"` (`gemini-3.8-flash`) for running commands (`cargo test`, `npm test`, `kubectl`, `helm`), applying edits, and parsing heavy command/test outputs.
